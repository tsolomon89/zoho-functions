...
| 2 | High → FIXED | **Demo reminder could never fire** — `sendDemoReminder` guarded on `Deal.Demo_Status`, which doesn't exist; `handleMeetingEvent` first-link dedup keyed off non-existent `Deal.Demo_Meeting_ID`. | FIXED: Meeting source-of-truth refactor; guards on Deal mirrors written from the Event; stage-rank first-booking. **Follow-up (2026-06-19):** also removed the residual dropped `Demo_Status` writes from `handleDemoOutcome` (6) — `Event.Meeting_Status` is the canonical demo lifecycle; no `Demo_Status` field exists anywhere. **Demo path retired (2026-06-19):** WF005 + the `handleDemoOutcome` automation record were deleted (Demo_Outcome is an off-layout Deal field, so the Deal-direct path could never fire). `handleMeetingEvent` (WF007) is now the **sole** demo transition owner; `Deal.Demo_Outcome` is kept only as an optional reporting mirror. Delete the `handleDemoOutcome` Deluge code unit in Dev Hub. |
