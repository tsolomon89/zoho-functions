...
- **Account-driven tier refresh:** `WF001c Process Account` (Accounts, create_or_edit, no criteria) fires `processAccount` on any Account create/edit → `processAccount` reconciles the canonical open Deal via `processDeal`. So setting/changing `Account.Company_Tier` re-runs the Amount hierarchy and updates a pre-Quote Deal's pipeline Amount. §8a2 mirrors the tier onto the Deal: when `Deal.Company_Tier` is blank it is copied from the Account (so `Contract_Target_ACV` resolves and the field shows the tier); when both are populated and differ, the Deal tier is PRESERVED and `[company_tier_conflict]` is raised (never a silent overwrite). Documented precedence: once a Deal tier is set it is authoritative/sticky — a later differing Account tier surfaces as a conflict review rather than auto-repricing; clear the Deal tier to re-pull from the Account. `processLead` seeds Lead→Deal on conversion; §8a2 seeds Account→Deal thereafter.
