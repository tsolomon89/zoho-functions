...
`Opportunity_State=Open` + `Opportunity_Status=New` — Deals have **no** bare `State`/`Status`); Deal
`Opportunity_Stage = Marketing Consent` (live label), `Stage=MQL`; Contact_Roles has
...
`Sequence_Step=1`, **`Status=Working`** (Running ⇒ Working; `State` stays Open); one
`Email Sent: marketing-consent:1:initial` audit Task; one open `Marketing Consent Call 1`. The Deal rolls
to `Opportunity_Status=Working` (`Opportunity_State` stays Open) once the Contact is Running.
Repeat with `=Call` → `activate:call` (Call 1, **no** opener email).
...
Quote. **Deal is never "Won":** `Opportunity_State` stays `Open` (a signed deal advances
`Opportunity_Stage` through Onboarding/Renewal — it does not become Won). **Pass = `Quote_Stage`
(+ commercial inputs) only; ledger fields automation-owned.**
...
**Assert:** the acting Contact is Lost (a/b `Lost_Reasons=No Fit`), but the **Deal stays Open**
(`Opportunity_State=Open`) because viability checks `Opportunity_State`/`Opportunity_Status` and finds C2 /
another active Quote, and a **Manual Review** "review primary / replacement possible" task is raised. Repeat
with C2 also closed → Deal closes Lost (`Opportunity_State=Lost`, `Opportunity_Status=Closed`). **Pass = loss
is one field; Deal viability is automation-owned + fail-safe.**
...
- `Renewal Declined` → Deal `Commercials_Status=Rejected` → `commercial:rejected` → acting Contact Lost
  (`Lost_Reasons=Terms Rejected`); **Deal stays Open** (`Opportunity_State=Open`) if another viable Contact
  remains, else closes Lost (`Opportunity_State=Lost`, `Opportunity_Status=Closed`).
